(() => {
    ...
    const items = document.querySelectorAll(".gallery__cont");
    if (window.matchMedia("(min-width: 768.9px)").matches) items.forEach((item => {
        item.dataset.prlxParent = true;
    })); else items.forEach((item => {
        delete item.dataset.prlxParent;
    }));
    class Parallax {
        ...
        animationFrame() {
            ...
            this.elements.forEach((el => {
                const parameters = {
                    ...
                    coefficient: el.dataset.coefficient ? Number(el.dataset.coefficient) : 9,
